...
<%-- 에러페이지를 'badNumber.jsp'로 설정 --%>
<%@ page errorPage="badNumber.jsp"%>

<%-- GuessGameLogic Bean을 session scope로 초기화 --%>
<jsp:useBean id="gameService" scope="session"
	class="guess.game.GuessGameLogic">
	<%
...
<BODY BGCOLOR='white'>
	...
	<%
			...
			%>
			<jsp:forward page="gameResult.jsp">
				<jsp:param value="you_win" name="result" />
			</jsp:forward>
			<%
		}
	...
	%>
	<jsp:forward page="gameResult.jsp">
		<jsp:param value="you_lose" name="result" />
	</jsp:forward>
	<%
		// 기회가 남았을 경우 남은 횟수 출력
			} else {
	%>
	<P>
		<FONT COLOR='red'> Your guess, <%=guess%>, was not correct.
		</FONT> <BR /> You have
		<%=gameService.getHowManyGuessesLeft()%>
		guesses left.
	</P>
	<%
		}
	...
	<FORM ACTION='guessGame.jsp' METHOD='POST'>
		What's your guess? <INPUT TYPE='text' NAME='guess' /><BR /> <BR />
		<INPUT TYPE='submit' />
	</FORM>
</BODY>
</HTML>
